...
export function KatalogMenMain({id}){
    ...
    const cart = datesAll.tabsImgMen[id]
        ...
        <div className="KatalogMenMain KatalogAllMain">
            <article className="KatalogAllArticle">
                ...
                <div className="Katalog_block2">
                    <img className="Katalog_block2Img" src="https://i.postimg.cc/sgZbJmsH/rejting-rkswlht9y8ep-128.png" alt="img"/>
                    ...
                    <article className="Katalog_block2BuyBlock">
                        <button className="Katalog_block2Btn" onClick={() => AddItemBasket(cart)}>В корзину</button>
                        <img className="Katalog_block2Img1" onClick={() =>  AddItemfavorites(cart)} src="https://heat-hit.ru/wp-content/uploads/2023/03/ic-heart-1320x1320.png" alt=""/>
                    </article>
                    <div className="specifications_titleBlock">
                    <h3 className="specifications_title" onClick={() => SetSpecifications(!specifications)}>Характеристика</h3>
                    {specifications && <Specifications cart={cart}/>}
                    </div>
                    <div className="specifications_titleBlock">
                    <h3 className="specifications_title" onClick={() => SetReviews(!reviews)}>Отзывы</h3>
                    {reviews && <div><p className="SpecificationsText">Отзывов нет</p></div>}
                    </div>
                </div>
            </article>
            <article className="KatalogSimilarBlock">
                ...
                {datesAll.tabsImgMen.map((item,index) => (
                    <article className="KatalogSimilarBlock2" key={index}>
                        <Link to={`/katalog/Men/${index}`}><img className="KatalogSimilarImg" src={item.url} alt=""/></Link>
                        <h3 className="KatalogSimilarText" key={item.title}>{item.title}</h3>
                        <p className="KatalogSimilarPrice" key={item.price}>{item.price} ₽</p>
                        <article className="KatalogSimilarBlock3">
                        <Link to={`/katalog/Men/${index}`} className="KatalogSimilarBtn"><button className="KatalogSimilarBtnA">Подробнее</button></Link>
                            <img className="KatalogSimilarImg1" onClick={() =>  AddItemfavorites(item)} src="https://heat-hit.ru/wp-content/uploads/2023/03/ic-heart-1320x1320.png" alt=""/>
                        </article>
                    </article>
                ))}
                </div>
